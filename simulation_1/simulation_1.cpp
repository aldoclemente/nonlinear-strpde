...
int main(int argc, char* argv[]){
   ...
   /*
   for(int i = 1; i < 5; ++i){
   for(auto it = unit_square.boundary_begin(i); it != unit_square.boundary_end(i); ++it) {
    std::cout << "(" << (*it).node(0)[0]  << ", " << (*it).node(0)[1] << "); (" << (*it).node(1)[0] << ", " << (*it).node(1)[1]  << ") \t"<< (*it).marker() << std::endl; 
   ...
   */
   ...
   int n_observations = read_TXT<int>(datadir + "n_locations.txt")(std::stoi(argv[1]),0); // 0, ..., 4
   int nsim = 30;
   ...
   vector2txt(std::vector<int>{model.opt_.n_iter()}, datadir + std::to_string(n_observations) + "/" + 
