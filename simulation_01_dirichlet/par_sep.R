...
eval.locs = as.matrix(read.csv("../simulation_0_dirichlet/data/eval_locs.csv")[,2:3])
datadir = "data/"
...
nsim = 30 # 30
lambda = as.matrix(read.table("../simulation_0_dirichlet/data/lambda.txt",
                              header = F))
...
for(k in 1:length(NLOCS)){
  ...
  for(l in 1:length(lambda)){
    ...
    for(i in 0:(nsim-1)){
      ...
      exact = as.matrix( read.table(paste0(datadir, "exact_", i , ".txt")))
      test.evals = as.vector(eval.parabolic(exact, mesh, eval.locs, time_locs)[,2:n_time_locs])
      
      #IC = as.matrix( as.vector(read.table(paste0(datadir, "exact_", i ,".txt"))[,1]))
