...
public:
  ...
  VectorType u_init_;
  double tol_ = 1e-5;
  // BFGS<Eigen::Dynamic, BacktrackingLineSearch> opt_ = BFGS<Eigen::Dynamic,
  // BacktrackingLineSearch>(50, tol_, 0.01);
...
int main(int argc, char *argv[]) {
  ...
  // STRPDE - PARABOLIC
  /*
  Eigen::VectorXd par_time_locs =
      time_locations.tail(time_locations.size() - 1);
  Eigen::VectorXd par_obs =
      observations.tail(par_time_locs.size() * locations.rows());

  std::cout << par_time_locs << std::endl;
  Triangulation<1, 1> T(par_time_locs);
  GeoFrame data(unit_square, T);
  auto &l = data.insert_scalar_layer<POINT, POINT>(
      "layer", std::pair{locations, MESH_NODES});
  l.load_vec("y", par_obs);
  std::cout << l << std::endl;
