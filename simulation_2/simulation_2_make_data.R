...
for(e in 1:nrow(mesh$triangles)){
  ...
  if( dist(point, t(centers[r,]) )  <= sqrt(2) * 0.0625   ){
...
exact  = as.matrix(read.table("../simulation_1/data/exact.txt", header=F))
time_locs = as.matrix(read.table("../simulation_1/data/time_locations.txt"))
