...
struct fe_fisher_kpp {
    ...
    void init_areal(){
    	...
    	FeSpace Vh(domain_, P1<1>);
  		...
  		auto a = integral(domain_)(mu_*dot(grad(uh), grad(vh)) - alpha_*uh*vh);
   		...
   		const auto& [psi, measure_vec] = internals::areal_basis_eval(Vh,incidence_matrix_);
    ...
    fe_fisher_kpp(const Triangulation<2,2>& domain, double mu, double alpha,
    			  const BinaryMatrix<Dynamic>& incidence_matrix, const VectorType& time_locations, const VectorType& observations): domain_(domain), mu_(mu), alpha_(alpha), 
    			  incidence_matrix_(incidence_matrix), time_locations_(time_locations),dT_(time_locations[1] - time_locations[0]),
    			  observations_(observations), n_locs_(incidence_matrix.rows()), n_time_locs_(time_locations.size())
    			  {
    			  	init_areal();
    			  }
    ...
    VectorType adjoint(const VectorType& y) const{
    	...
    	for(int t = n_time_locs_ - 1; t > 0; t--){
    		...
    		VectorType b = 1./dT_*M_*get_t(p,t) - 1./(n_time_locs_*n_locs_)*PsiTPsi_* get_t(y,t-1) + 
    											  1./(n_time_locs_*n_locs_)*Psi_.transpose() * D_ *obs(t-1); // !!!!
    		//dof_handler.enforce_constraints(S,b);
    		dof_handler.enforce_constraints(b);
   ...
   VectorType p() const { 
	...
	VectorType u_init_;
	double tol_ = 1e-3;
	BFGS<Eigen::Dynamic, BacktrackingLineSearch> opt_ = BFGS<Eigen::Dynamic, BacktrackingLineSearch>(100, tol_, 0.01);
	//GradientDescent<Eigen::Dynamic, BacktrackingLineSearch> opt_ = GradientDescent<Eigen::Dynamic, BacktrackingLineSearch>(500, tol_, 0.01);
};
...
int main(int argc, char* argv[]){
   ...
   unit_square.mark_boundary(/* as = */ 1, /* where = */ [](const typename Triangulation<2, 2>::EdgeType& edge) {
        return (edge.node(0)[1] == 0.0 && edge.node(1)[1] == 0.0);     // bottom side ( y = 0 )
   });
   unit_square.mark_boundary(/* as = */ 2, /* where = */ [](const typename Triangulation<2, 2>::EdgeType& edge) {
        return (edge.node(0)[0] == 1.0 && edge.node(1)[0] == 1.0);     // right side ( x = 1 )
   });
   unit_square.mark_boundary(/* as = */ 3, /* where = */ [](const typename Triangulation<2, 2>::EdgeType& edge) {
        return ( edge.node(0)[1] == 1.0 && edge.node(1)[1] == 1.0 );     // upper side ( y = 1)
   });
   unit_square.mark_boundary(/* as = */ 4, /* where = */ [](const typename Triangulation<2, 2>::EdgeType& edge) {
        return (edge.node(0)[0] == 0.0 && edge.node(1)[0] == 0.0);     // left side ( x = 0 )
   });
  
   double mu = 0.1;
   double alpha = 1.0;
   
   Eigen::MatrixXi incidence_matrix = read_TXT<int>(datadir + "incidence_matrix.txt"); 
   ...
   Eigen::MatrixXd obsMat = read_TXT<double>(datadir + std::to_string(sim) + "/" + "obs.txt");
   
   std::cout << ":)" << std::endl;
   
   Eigen::VectorXd observations = Eigen::Map<Eigen::VectorXd>(obsMat.data(), obsMat.size());
   std::cout << "obs "<<  observations.rows() << " " << observations.cols() << std::endl;
   
   Eigen::VectorXd time_locations = read_TXT<double>( "../simulation_1/" + datadir + "time_locations.txt");
   
   std::cout << time_locations.rows() << " " << time_locations.cols() << std::endl;
   
   auto model = fe_fisher_kpp(unit_square, mu, alpha, bm, time_locations, observations);
   
   Eigen::VectorXd IC = read_TXT<double>("../simulation_1/" + datadir + "exact.txt").col(0);
   ...
   Eigen::VectorXd u0 = read_TXT<double>("../simulation_1/" + datadir + "u_guess_rand.txt");
   ...
   vector2txt(std::vector<int>{model.opt_.n_iter()}, datadir + std::to_string(sim) +"/" + "n_iter.txt");
