...
time_locs = as.matrix(read.table(paste0("../simulation_0_dirichlet/data/time_locations.txt")))
...
f_exact =  as.matrix(read.table(paste0("../simulation_0_dirichlet/data/exact.txt")))
...
} 
{
  ...
  for(t in 1:n_time_locs){
    ...
    plt_rect <- plt + geom_rect(data = data.frame(xmin = -2.5,ymin = -2.5,xmax = +2.5,ymax = +2.5), 
                                aes(xmin = xmin, ymin = ymin, xmax = xmax, ymax = ymax),
                                fill = "transparent", 
                                color = "black",
                                linewidth = 1)
