...
public:
  ...
  fe_fisher_kpp(const Triangulation<3, 3> &domain, double mu, double alpha,
  ...
  std::function<VectorType(const VectorType&)> gradient() {
    return [this](const VectorType &u) {
      VectorType y = state(u);
      VectorType p = adjoint(y);
      VectorType grad_u = lambda_ * Mt_ * u - Mt_ * p; // qua ci va lambda e non alpha (ci andrebbe un dT...)
...
//   }
  ...
  // attribute
  const Triangulation<3, 3> &domain_;
...
int main(int argc, char *argv[]){
    ...
    std::string data_dir = "mesh/";
    std::string mesh_dir = data_dir;
    
    std::string output_dir = "output/";
    std::string command_str = "mkdir -p " + output_dir;
    system(command_str.c_str());

    auto nodes = read_mtx<double>(mesh_dir + "nodes.16.mtx");
    auto cells = read_mtx<int>(mesh_dir + "elements.16.mtx");
    auto boundary = read_mtx<int>(mesh_dir + "boundary.16.mtx");
    ...
    matrix_t time_mesh = matrix_t::Zero(4,1);
    time_mesh << 0.0, 1.0, 2.0, 3.0;
    ...
    matrix_t obs = matrix_t::Zero(nodes.rows(), time_mesh.size());
    for(int i = 0; i < time_mesh.size(); ++i){
        obs.col(i) = read_mtx<double>(data_dir + "035_S_4114."+ std::to_string(i) + ".mtx");
    }
    
    double mu = 1e-6; // ... tipici valori ... ma forse andrebbe scelto anche lui in 10-fold cross validation
    ...
    auto mass = integral(brain)(u*v).assemble();
    // IC estimate -----------------------------------------------------------------------------------------------
    auto a_IC = integral(brain)(dot(grad(u), grad(v))); 
    ...
    GeoFrame data_IC(brain);
    auto &l_IC = data_IC.insert_scalar_layer<POINT>("layer", MESH_NODES);
    ...
    vector_t IC = elliptic.f();
    std::cout << "||IC||_L2^2 " << (IC.transpose() * (mass*IC))(0,0)<< std::endl;
    ...
    ScalarField<2> SSE;
    SSE = [&brain, &cv_, &time_mesh, &IC, &Vh, &mu, &n_times, &n_dofs, &g_init, &Im, &T, &a, &F, &time_locs](const vector_t& grid) -> double {
        ...
        for(int k=0; k < cv_.k(); ++k){
            ...
            if( alpha > 0){
                auto model = fe_fisher_kpp(brain, mu, alpha, locs, time_mesh,
                                 obs.reshaped());
    ...
    auto model = fe_fisher_kpp(brain, mu, optimum[1], locs, time_mesh,
                             obs.reshaped());
    ...
    optimizer.optimize(SSE, grid_para); 

    vector_t values_para = vector_t::Zero(optimizer.values().size()); 
    for(int i = 0; i < optimizer.values().size(); ++i) values_para[i] = optimizer.values()[i];
    optimum << optimizer.optimum()[0], optimizer.optimum()[1]; 
    
    Eigen::saveMarket(values_para, output_dir + "cv_errors_diff.mtx");
    // Eigen::Index minRow;
    // values_para.minCoeff(&minRow);
    // double lambda_para = lambdas[minRow];
    std::cout << "opt (para) " << optimum[0] << std::endl;
    std::cout << "values (para) " << values_para << std::endl;
    // ---- output kFold 

    vector_t response = obs.rightCols(n_times - 1).reshaped();
    GeoFrame data(brain, T);
    auto &l = data.insert_scalar_layer<POINT, POINT>("layer", std::pair{MESH_NODES, time_locs});
    l.load_vec("y", response);
    SRPDE parabolic("y ~ f", data, fe_ls_parabolic_mono(std::pair{a, F}, IC));
