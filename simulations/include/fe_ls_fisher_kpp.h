...
class fe_ls_fisher_kpp{
 private:
   ...
   public:
    ...
    fe_ls_fisher_kpp(const std::string& formula, const GeoFrame& gf, PDEparam&& pde_param, const WeightMatrix& W) :
        fe_space_() {
    ...
    template <typename GeoFrame, typename PDEparam, typename WeightMatrix>
    fe_ls_fisher_kpp(const GeoFrame& gf, PDEparam&& pde_param, const WeightMatrix& W) : fe_space_(), W_(W) {
        fdapde_static_assert(GeoFrame::Order == 2, THIS_CLASS_IS_FOR_ORDER_TWO_GEOFRAMES_ONLY);
	fdapde_assert(gf.n_layers() == 1);
	n_obs_  = gf[0].rows();
	n_locs_ = n_obs_;
    ...
    void analyze_data(const std::string& formula, const GeoFrame& gf, const WeightMatrix& W) {
        ...
        // update forcing
        u0_ = u_.topRows(n_dofs_) + (1.0 / DeltaT_) * (M_ * s_);
    ...
    struct obj_t {
        ...
        obj_t(fe_ls_fisher_kpp& model, double lambda, double tol) :
            model_(std::addressof(model)), lambda_(lambda), tol_(tol) { }
        ...
        double operator()(const vector_t& g__) const {
            ...
            // pensare a modo per evitare ste cope stupide? 
          ...
          int max_iter, double tol) :
            ...
            if constexpr (std::is_same_v<Diffusion, Scalar>) {
                ...
                for (int i = 0; i < local_dim; ++i) {
                    for (int j = 0; j < local_dim; ++j) { mu(i, j) = diffusion(i, j); }
                }
                diffusion_mtx = mu.reshaped().transpose().replicate(n_quadrature_nodes, 1);
                diffusion_ = diffusion_t(diffusion_mtx);
