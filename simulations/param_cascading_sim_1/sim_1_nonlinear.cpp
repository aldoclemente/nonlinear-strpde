...
int main(int argc, char *argv[]){
    ...
    size_t i = 0;
    ScalarField<1> PC;
    PC = [&unit_square, &data, &IC, &mu, &lambda_grid, &i, &lbfgs, &g_init](vector_t x) -> double {
        ...
        SRPDE nonlinear("y~f", data, fe_ls_fisher_kpp(std::pair(mu,x(0,0)), Vh, IC)); 
        
        // inner fitting criterion (fixed lambda)
        nonlinear.fit(lambda_grid.row(i)[0], g_init, lbfgs);
        
        // outer opt criterion
        std::cout << "SSE: " << ((nonlinear.fitted() - nonlinear.response()).squaredNorm()) << std::endl;
        return ((nonlinear.fitted() - nonlinear.response()).squaredNorm());
    };
    ...
    // auto step() = [&unit_square, &data, &IC, &mu, &lambda_grid, &i, &alpha_optim, &optimizer] () -> double {
    //     FeSpace Vh(unit_square, P1<1>);
    //     SRPDE nonlinear("y~f", data, fe_ls_fisher_kpp(std::pair(mu,alpha), Vh, IC)); 
        
    //     return nonlinear.gcv(100, 476813)(lambda_grid.row(i));
    // };
