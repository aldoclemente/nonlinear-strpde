...
public:
  ...
  fe_fisher_kpp(const Triangulation<2, 2> &domain, double mu, double alpha,
                const BinaryMatrix<Dynamic> &incidence_matrix, const VectorType &time_locations,
  ...
  VectorType adjoint(const VectorType &y) const {
    ...
    for (int t = n_time_locs_ - 1; t > 0; t--) {
      ...
      VectorType b =
          ...
          1. / (n_time_locs_ * n_locs_) * Psi_.transpose() * D_ * observations_.block(n_locs_ * (t-1), 0, n_locs_, 1);
...
int main(int argc, char *argv[]){
    ...
    std::string output_dir = "output/"  + sim + "/";
    std::string command_str = "mkdir -p " + output_dir;
    system(command_str.c_str());
    auto test_locs = read_mtx<double>(mesh_dir + "test_locs.mtx");
    ...
    matrix_t obs = read_mtx<double>(sim_dir + "obs.cpp.mtx");
    ...
    SSE = [&unit_square, &cv_, &time_mesh, &IC, &Vh, &mu, &n_times, &n_dofs, &g_init, &Im, &T, &a, &F, &time_locs](const vector_t& grid) -> double {
        ...
        for(int k=0; k < cv_.k(); ++k){
            ...
            if( alpha > 0){
                auto model = fe_fisher_kpp(unit_square, mu, alpha, bm, time_mesh,
                                 obs.reshaped());
    ...
    vector_t result = vector_t::Ones((n_times-1)*n_dofs);
    ...
    sparse_matrix_t Psi_test = kronecker(Im, psi_test);
    auto selected_reac = optimum[1];
    if(selected_reac > 0){
        auto model = fe_fisher_kpp(unit_square, mu, optimum[1], bm, time_mesh,
                                 obs.reshaped());
    ...
    double lambda_para = lambdas[minRow]; // era sbagliato........
