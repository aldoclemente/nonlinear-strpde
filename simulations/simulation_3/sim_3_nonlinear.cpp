...
int main(int argc, char *argv[]){
    ...
    matrix_t time_mesh = read_mtx<double>(data_dir2 + "time_mesh.mtx");
    ...
    Eigen::Matrix<int, Dynamic, Dynamic> incidence_matrix = read_csv<int>("../data/simulation_3/incidence_matrix.csv").as_matrix();
    matrix_t obs = read_mtx<double>(sim_dir + "obs.mtx");
     
    vector_t IC = read_mtx<double>(sim_dir2 + "exact.mtx").col(0);
    ...
    auto f_ = read_mtx<double>(sim_dir + "estimate_nonlinear.mtx");
    auto psi_test = internals::point_basis_eval(Vh, test_locs);

    matrix_t test_vals = matrix_t::Zero(test_locs.rows(), n_times - 1); // t0 butto via
    for(int t = 0; t<n_times-1; ++t){ 
        //test_vals.col(t) = psi_test * nonlinear.f().block(n_dofs*t, 0, n_dofs, 1);
        test_vals.col(t) = psi_test * f_.block(n_dofs*t, 0, n_dofs, 1);    
    }

    matrix_t test_obs = read_mtx<double>(sim_dir2 + "test_obs.mtx");
